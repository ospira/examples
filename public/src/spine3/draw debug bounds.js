class Example extends Phaser.Scene
{
    ...
    create ()
    {
        const b = this.add.spine(500, 600, 'set1.greengirl').setScale(0.7);

        b.drawDebug = true;

        const anims = b.getAnimationList();
    ...
    createToggle (x, y, label, enabled, callback)
    {
        const button = this.add.image(x, y, (enabled) ? 'on' : 'off').setOrigin(0).setScrollFactor(0);

        const text = this.add.text(x + 90, y + 6, label, { font: '16px Courier', fill: '#ffffff' }).setShadow(1, 1).setScrollFactor(0);
